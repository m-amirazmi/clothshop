...
const ProductRating: React.FunctionComponent<ProductIRatingProps> = ({ rating }) => {
	const renderRating = () => {
		const _icon = [];
		for (let x = 0; x < 5; x++) {
			if (rating - x <= 0.5 && rating - x >= 0.1) _icon.push(<BsStarHalf key={x} className="text-accent-3" />);
			else if (x < rating) _icon.push(<BsStarFill key={x} className="text-accent-3" />);
			else _icon.push(<BsStar key={x} className="text-accent-3" />);
		}
		return (
